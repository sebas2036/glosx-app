    ...
    const TRANSLATIONS = {
      en: {
        ...
        disc_cta_title: 'Today we\'re going to…?',
        ...
        ai_input_ph: 'Example: Zurich to Milan', ai_suggest_label: 'Popular pairs:',
      ...
      es: {
        ...
        disc_label: 'Comunidad', disc_title1: 'Descubramos', disc_title2: 'juntos.', disc_lead: 'Pueblos y rincones de Europa que solo encuentras en tren — compartidos por viajeros, para viajeros.', disc_cta_title: '¿Hoy nos vamos a…?', disc_cta_text: 'Gira la ruleta: un par real y el itinerario se arma solo.', disc_cta_btn: 'Gira la ruleta', disc_cta_again: 'Elige otra', disc_cta_this: 'Me quedo',
        ...
        faq2_q: '¿Cómo reservo un billete?', faq2_a: 'Te conectamos de forma directa y transparente con socios oficiales como Klook. Busca tu ruta, haz clic en "Ver horarios →" y completa la reserva en la plataforma segura de Klook.',
        ...
        disc_cta_title: '¿Hoy nos vamos a…?',
        reviews_label: 'Historias del Vagón', reviews_title1: 'Voces del', reviews_title2: 'trayecto.',
        reviews_lead: 'Lo que recuerdan no es el destino — es el momento en que el tren lo cambió todo.', reviews_disclaimer: 'Las tarjetas de abajo son ejemplos ilustrativos. Comparte tu momento WoW real abajo — se publica luego de una revisión rápida.',
      ...
      fr: {
        ...
        ai_input_ph: 'Exemple : Zurich à Milan', ai_suggest_label: 'Trajets populaires :',
      ...
      de: {
        ...
        disc_label: 'Community', disc_title1: 'Entdecken wir', disc_title2: 'gemeinsam.', disc_lead: 'Orte und Winkel Europas, die man nur mit dem Zug findet — geteilt von Reisenden, für Reisende.', disc_cta_title: 'Heute fahren wir nach…?', disc_cta_text: 'Dreh das Rad: ein echtes Städtepaar, der Plan entsteht von selbst.', disc_cta_btn: 'Rad drehen', disc_cta_again: 'Noch mal', disc_cta_this: 'Ich bleibe',
        ...
        disc_cta_title: 'Heute fahren wir nach…?',
        ...
        ai_input_ph: 'Beispiel: Zürich nach Mailand', ai_suggest_label: 'Beliebte Paare:',
      ...
      it: {
        ...
        disc_cta_title: 'Oggi partiamo per…?',
        ...
        ai_input_ph: 'Esempio: Zurigo a Milano', ai_suggest_label: 'Tratte popolari:',
      ...
      pt: {
        ...
        disc_cta_title: 'Hoje vamos para…?',
        ...
        ai_input_ph: 'Exemplo: Zurique a Milão', ai_suggest_label: 'Pares populares:',
...
(function () {
  ...
  function cityRouletteLabel(slug) {
    const lang = document.documentElement.lang || 'en';
    const c = (window._GLOSX_CITIES || []).find(function (x) { return x.slug === slug; });
    if (!c) return String(slug || '').replace(/-/g, ' ').replace(/\b\w/g, function (m) { return m.toUpperCase(); });
    const parts = String(c.display || '').split(' / ');
    if ((lang === 'en' || lang === 'de') && parts.length > 1) return parts[parts.length - 1];
    return parts[0];
  }

  function roulettePairs() {
    const cities = window._GLOSX_CITIES || [];
    const slugs = cities.map(function (c) { return c.slug; }).sort(function (a, b) { return b.length - a.length; });
    const pages = window.GLOSX_ROUTE_PAGES;
    const out = [];
    if (!pages) return out;
    pages.forEach(function (key) {
      for (let i = 0; i < slugs.length; i++) {
        ...
        if (key.indexOf(from + '-') === 0) {
          const to = key.slice(from.length + 1);
          if (cities.some(function (c) { return c.slug === to; })) {
            out.push([from, to]);
            return;
          }
        }
  ...
  function spinPairRoulette() {
    ...
    function pick() {
      ...
      while (pairs.length > 1 && (p[0] + '-' + p[1]) === _rouletteLast);
      return p;
    }
    function lock(p) {
  ...
  window.spinPairRoulette = spinPairRoulette;
