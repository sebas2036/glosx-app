    ...
    const TRANSLATIONS = {
      en: {
        ...
        ai_hotel_link: 'Find hotels →', ai_hotel_price: 'See current price →', ai_kiwi_cta: 'Transfer on arrival to', ai_stop_label: 'STOP', ai_budget_label: 'Estimated Trip Budget', ai_budget_trains: 'Trains (this route)', ai_budget_daily: 'Hostel + food, per day', ai_budget_disclaimer: 'Estimates only, based on public budget-travel averages — actual prices vary by provider, season and booking date. Always confirm final pricing before purchasing.',
      ...
      fr: {
        ...
        ai_hotel_link: 'Voir les hôtels →', ai_hotel_price: 'Voir le prix actuel →', ai_kiwi_cta: 'Transfer à l\'arrivée à', ai_stop_label: 'ARRÊT', ai_budget_label: 'Budget de voyage estimé', ai_budget_trains: 'Trains (cet itinéraire)', ai_budget_daily: 'Auberge + repas, par jour', ai_budget_disclaimer: 'Estimations uniquement, basées sur des moyennes publiques de voyage à petit budget — les prix réels varient selon le fournisseur, la saison et la date de réservation. Confirmez toujours le prix final avant d\'acheter.',
...
(function () {
  ...
  // Rangos de referencia (hostel/dia y comida/dia) por nivel de costo de pais,
  // basados en promedios publicos tipo Budget Your Trip / Hostelz Price Index
  // (no es una API en vivo, son valores de referencia que se actualizan a mano).
  const COUNTRY_BUDGET_TIER = {
    high:   { hostel: [25, 42], food: [20, 35] }, // UK, Suiza, Francia, Paises Bajos, escandinavia, Monaco
    mid:    { hostel: [18, 30], food: [15, 25] }, // Alemania, Austria, Italia, Belgica, Espana
    budget: { hostel: [12, 22], food: [10, 18] }, // Chequia, Hungria, Portugal
  };
  const COUNTRY_TIER = {
    'United Kingdom': 'high', 'Switzerland': 'high', 'France': 'high', 'Netherlands': 'high',
    'Denmark': 'high', 'Sweden': 'high', 'Norway': 'high', 'Monaco': 'high',
    'Germany': 'mid', 'Austria': 'mid', 'Italy': 'mid', 'Belgium': 'mid', 'Spain': 'mid',
    'Czech Republic': 'budget', 'Hungary': 'budget', 'Portugal': 'budget', 'Poland': 'budget',
  };
  // CITY_TO_COUNTRY se arma a partir del mismo listado que ya alimenta el menu de hoteles Klook
  ...
  function renderBudgetCard(data) {
    const el = document.getElementById('aiBudgetCard');
    if (!el) return;
    const est = computeBudgetEstimate(data);
    const dict = (TRANSLATIONS[document.documentElement.lang] || TRANSLATIONS.en);
    el.innerHTML = `
      <p class="ai-budget-label">${dict.ai_budget_label || 'Estimated Trip Budget'}</p>
      <div class="ai-budget-rows">
        <div class="ai-budget-row">
          <span>${dict.ai_budget_trains || 'Trains (this route)'}</span>
          <strong>€${est.trainLow}–${est.trainHigh}</strong>
        </div>
        <div class="ai-budget-row">
          <span>${dict.ai_budget_daily || 'Hostel + food, per day'}</span>
          <strong>€${est.dailyLow}–${est.dailyHigh}</strong>
        </div>
      </div>
      <p class="ai-budget-disclaimer">${dict.ai_budget_disclaimer || 'Estimates only, based on public budget-travel averages — actual prices vary by provider, season and booking date. Always confirm final pricing before purchasing.'}</p>
    `;
  }
